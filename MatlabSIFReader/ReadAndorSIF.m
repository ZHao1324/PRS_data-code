function data = ReadAndorSIF(FileName)
% % calibvals, x axis
% % data, y axis
...
if (rc == 22002)
  ...
  if present
    ...
    if (no_frames > 0)
        ...
        data = zeros(size, no_frames);
        for ij = signal:no_frames-1
            [rc, datatemp] = atsif_getframe(signal, ij, size);
            data(:, ij+1) = datatemp;
        end
...
%            ylabel({ytype;yunit});
        elseif(pattern == '4')
            ...
            data = squeeze(reshape(data, width, height, no_frames));
%            imagesc(newdata);
        end
    end    
  end
