function varargout=notBoxPlot_zach2(y,x,zachC,jitter,style,mufac)
...
if nargin<3 || isempty(zachC)
    zachC=[1 0 0]; %default red
end
...
if nargin<6
  mufac=1; %Can also be 'line' or 'sdline'
end
...
set([h.data],'Color',min([1 1 1;(zachC+0.3)],[],1),'MarkerFaceColor',[0.6 0.6 0.6]);
set([h.mu],'Color',zachC);
set([h.sdPtch],'FaceColor',zachC,'FaceAlpha',0.4);
set([h.semPtch],'FaceColor',zachC,'FaceAlpha',0.2)
...
SEM = 2*SD; % to plot 1 and 2x SD rather than SD and SEM!
...
for k=1:length(X)
    ...
    if strcmp(style,'patch') || strcmp(style,'sdline')
      ...
      h(k).mu=plot([X(k)-mufac*jitScale,X(k)+mufac*jitScale],[mu(k),mu(k)],'-r',...
           'linewidth',2);
    end
